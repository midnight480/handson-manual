...
Positive
: テンプレートリポジトリには開発環境の設定（devcontainer）が含まれており、AWS CLI、Python、Node.js などが自動的にインストールされます。
...
GitHub Codespaces やご自身の端末を利用する場合は、AWS リソースを操作するためのアクセスキー（認証情報）が必要です。以下の手順で IAM ユーザーのアクセスキーを発行し、設定してください。
...
開発環境（Codespaces またはご自身の端末）のターミナルで以下のコマンドを実行し、先ほど発行したキーを設定します。
