...
3. **Directory Renaming**:
   - By default, `claat export` will create a new directory named after the markdown file's Codelab ID (e.g., creating `xxx/index.html` and `xxx/css` if the ID is `xxx`).
   - **CRITICAL**: This project requires the final output directory to be named exactly `web`.
   - After running the export, you **MUST** change the generated directory to `web`.
