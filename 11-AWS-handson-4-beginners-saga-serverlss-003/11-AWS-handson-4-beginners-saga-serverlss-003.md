...
ハンズオン用に作成した IAM ユーザーを削除します。

1. IAMコンソールで **ユーザー** → `handson-user` を選択
2. **削除** をクリック（アクセスキーも自動的に削除されます）

### GitHub Codespace の削除（任意）

不要であれば Codespace を削除します（GitHub の Codespaces 管理画面から）。
