...
When the user invokes AI-DLC, read and follow `.aidlc-rule-details/` for rule details, then follow the AI-DLC adaptive workflow below.

The rule details directory resolves in this order (use the first that exists):
- `.aidlc-rule-details/` (symlink to `.kiro/aws-aidlc-rule-details/`)
- `.kiro/aws-aidlc-rule-details/`

Start by reading `.aidlc-rule-details/common/process-overview.md` and follow the core workflow defined in the steering rules.
